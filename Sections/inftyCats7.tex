...
\end{proof}
The main theorem of this subsection is Stasheff's recognition principle for loop animae:
...
\end{rem}
As the black box suggests, we won't give a proof of \cref{thm:E1Loop}\cref{enum:E1LoopMon} (and we won't need it either), but at least we'll construct an $\IE_1$-monoid structure on the anima $\operatorname{End}_\Cc(x)\coloneqq\Hom_\Cc(x,x)$ of endomorphisms of $x$ and show that it fits into an adjunction
\begin{equation*}
...
Let us now turn to \cref{thm:E1Loop}\cref{enum:E1LoopGrp}. The proof will consist of two parts: a formal nonsense part, in which we effortlessly deduce the adjunction $\B\colon \cat{Mon}(\cat{An})\shortdoublelrmorphism \cat{An}_{*/}\noloc \Omega$ from \cref{lem:B+EndAdjunction}, and a hard part, in which we compute $\Omega\B G$ for every $\IE_1$-group $G$ to establish the equivalence $\cat{Grp}(\cat{An})\simeq (\cat{An}_{*/})_{\geqslant 1}$.
...
\begin{proof}[Proof sketch of \cref{thm:E1Loop}, formal nonsense part]
	...
	\begin{equation*}
		\begin{tikzcd}[column sep=large]
			\cat{Mon}(\cat{An})\rar[shift left=0.2em,"B^+"]\drar[end anchor=175,shorten <=0.4ex,shorten >=0.1ex,bend right=15.5,shift left=0.2em,"\B"] & (\cat{Cat}_\infty)_{*/}\lar[shift left=0.2em,"\operatorname{End}"]\dar[shift left=0.2em,"\left|\,\cdot\,\right|"]\dar[phantom,""{name=A}]\arrow[from=1-1,to=A,commutes,pos=0.7]\\%start anchor=325,end anchor=175,
...
\begin{thm}\label{thm:EinftyBOmegaEquivalence}
	...
	\begin{equation}\label{eq:BOmegaAdjunction}\tag{$*$}
		...
		\end{tikzcd}
	\end{equation}
	\embrace{note that $\ev_{\langle 1\rangle}\colon \cat{CMon}(\cat{An})\rightarrow \cat{An}$ factors canonically over $\cat{An}_{*/}\rightarrow \cat{An}$ since $\cat{CMon}(\cat{An})\simeq \cat{CMon}(\cat{An}_{*/})$ by \cref{con:BOmegaAdjunction}}. Furthermore:
	\begin{alphanumerate}
...
\begin{lem}\label{lem:HomotopyGroupsOfSpectra}
	\begin{alphanumerate}
...
\begin{exm}\label{exm:EilenbergMacLaneSpectra}
...
%	\begin{proof}
	...
	%		\begin{align*}
		%			\colimit\!\left(\Xx\times_\Yy\Yy_{/z}\rightarrow \Yy\xrightarrow{F\circ s} \Cc\right)&\longrightarrow F\lef(s(z)\righ)\simeq F\bigl(S^0\bigr)\quad\text{for all $z\in \Yy$}\,,\\
		%			\colimit\!\left(\Xx\times_\Yy\Yy_{/*}\rightarrow \Yy\xrightarrow{F\circ s}\Cc\right)&\longrightarrow F\lef(s(*)\righ)\simeq F(*)\,.
		%		\end{align*}
	...
	%		\begin{align*}
		%			\Hom_{\Xx\times_\Yy\Yy_{/z}}\lef((\alpha\colon y\rightarrow z),(\beta\colon y'\rightarrow z)\righ)&\simeq \Hom_\Yy^\mathrm{good}(y,y')\times_{\Hom_\Yy(y,z)}\{\alpha\}\simeq \emptyset\,,\\
		%			\Hom_{\Xx\times_\Yy\Yy_{/z}}\lef((\beta\colon y'\rightarrow z),(\alpha\colon y\rightarrow z)\righ)&\simeq \Hom_\Yy^\mathrm{good}(y',y)\times_{\Hom_\Yy(y',z)}\{\beta\}\simeq \emptyset\,.
		%		\end{align*}
	...
	%		\begin{equation*}
		%			\Hom_{\Xx\times_\Yy\Yy_{/z}}\lef((\gamma\colon y''\rightarrow z),(*\rightarrow z)\righ)\simeq \Hom_\Yy(y'',*)\times_{\Hom_\Yy(y'',z)}\{\gamma\}
		%		\end{equation*}
