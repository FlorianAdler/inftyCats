...
\begin{proof}
	...
	\begin{equation*}
		...
		\end{tikzcd}
	\end{equation*}
	is cofinal. But that's easy! Both $\Cc_{/x}$ and $\Dd_{/F(x)}$ have terminal objects, hence there are adjunctions $\Cc_{/x}\shortdoublelrmorphism \{\id_x\}$ and $\Dd_{/F(x)}\shortdoublelrmorphism\{\id_{F(x)}\}$. Hence $*\rightarrow \Cc_{/x}$ and $*\rightarrow \Dd_{/F(x)}$ are both cofinal by \cref{exm:Cofinal}\cref{enum:RightAdjointCofinal}. Since being cofinal is closed under $2$-out-of-$3$ (for example, by the condition from \cref{thm:JoyalsQuillenA}\cref{enum:Cofinal}), $\Cc_{/x}\rightarrow \Dd_{/F(x)}$ must be cofinal too.
\end{proof}
...
\begin{lem}\label{lem:FilteredCofinal}
	Let $\kappa$ be a regular cardinal and let $\Jj$ be a $\kappa$-filtered $\infty$-category. Then $\abs*{\Jj}\simeq *$. Furthermore, for every $j\in\Jj$ the slice $\Jj_{j/}$ is $\kappa$-filtered again and $\Jj_{j/}\rightarrow \Jj$ is cofinal.
\end{lem}
\begin{proof}[Proof sketch]
...
\end{lem}
\begin{proof}
	Clearly, $\const y\colon \Ii\rightarrow \Dd$ factors through $\Ii\rightarrow\abs*{\Ii}$. This functor is cofinal by \cref{exm:Cofinal}\cref{enum:LocalisationsCofinal}, and since $ \abs*{\Ii}\simeq *$, it follows that the colimit is indeed given by $y$. 
\end{proof}
...
\begin{alphanumerate}
	...
	\end{equation*}
	(to see this, just observe that these diagrams can be (co)finally replaced by constant $x$-valued diagrams). But a finite limit or colimit will never suffice.
...
\end{lem}
The proof needs a technical lemma that has already been referenced several times before.
...
\begin{proof}[Proof sketch of \cref{lem:PrLColimits}]
	...
	We start with products. Let $(\Cc_i)_{i\in I}$ be an collection of $\infty$-categories and let $\Cc\coloneqq \prod_{i\in I}\Cc_i$. We wish to show that if all $\Cc_i$ are presentable, then so is $\Cc$, and then $\Cc$ satisfies the universal property of the product in both $\cat{Pr}^\L$ and $\cat{Pr}^\R$ (compare this to footnote~\cref{footnote:LimitNonFullSubcategory} below). If we can show that $\Cc$ is accessible, then all the other desired properties follow easily from \cref{lem:HomInLimits}\cref{enum:ColimitsInLimits}. So let's show that products of accessible $\infty$-categories are accessible again. By \cref{lem:Accessible}, we can choose a sufficiently large regular cardinal $\kappa$ such that $\kappa >\abs*{I}$ and all $\Cc_i$ are $\kappa$-accessible. If $x=(x_i)_{i\in I}$ is an object of $\Cc$ such that each $x_i\in \Cc_i$ is $\kappa$-compact, then $x$ is $\kappa$-compact too: Indeed, this follows from \cref{lem:HomInLimits}\cref{enum:HomInLimits} and the fact that $\kappa$-filtered colimits commute with $I$-indexed products in $\cat{An}$ by \cref{lem:FilteredColimitsPreserveFiniteLimits}. Now let $y=(y_i)_{i\in I}$ be another object of $\Cc$. For all $i\in I$, we can write $y_i\simeq \colimit_{j\in\Jj_i}x_{i}(j)$ as a $\kappa$-filtered colimit of $\kappa$-compact objects. By the same trick as in the proof of \cref{lem:Presentable}, we can replace each $\Jj_i$ by $\Jj\coloneqq \prod_{i\in I}\Jj_i$, put $x(j)\coloneqq (x_i(j))_{i\in I}\in\Cc$ and then $y\simeq \colimit_{j\in \Jj}x(j)$ is expressible as a $\kappa$-filtered colimit of $\kappa$-compact objects by \cref{lem:HomInLimits}\cref{enum:ColimitsInLimits}.
	
	It remains to do pullbacks. Proving that pullbacks of accessible $\infty$-categories along functors that preserve sufficiently filtered colimits stay accessible is not quite easy and we'll refer to \cite[Proposition~\HTTthm{5.4.6.6}]{HTT}. If $\Cc_0\rightarrow \Cc_2\leftarrow \Cc_1$ are functors in $\cat{Pr}^\L$ or $\cat{Pr}^\R$, then they preserve sufficiently filtered colimits (for $\cat{Pr}^\R$ this needs \cref{lem:RightAdjointsAccessible}) and so the pullback $\Dd\coloneqq\Cc_0\times_{\Cc_2}\Cc_1$ is accessible. In the case of $\cat{Pr}^\L$, \cref{lem:HomInLimits}\cref{enum:ColimitsInLimits} shows that $\Cc$ has all colimits, so it is presentable, and that the projections $\pr_i\colon \Cc\rightarrow \Cc_i$ preserve all colimits, so they are functors in $\cat{Pr}^\L$. The required universal property in $\cat{Pr}^\L$ is then straightforward to verify. In the case of $\cat{Pr}^\R$, \cref{lem:HomInLimits}\cref{enum:ColimitsInLimits} shows that $\Cc$ has all limits, hence it is presentable by \cref{cor:PresentableComplete}\cref{enum:AccessibleCocomplete}, and that the projections $\pr_i\colon \Cc\rightarrow \Cc_i$ preserve all limits and sufficiently filtered colimits, so they are functors in $\cat{Pr}^\R$ by \cref{thm:AdjointFunctorTheorem}\cref{enum:AdjointFunctorTheoremRight}. Again, the required universal property in $\cat{Pr}^\R$ is then straightforward to verify.
\end{proof}
...
\begin{thm}[\enquote{Russel's paradox? Skill issue!}]\label{lem:Mindblow}
