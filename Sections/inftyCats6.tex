...
\begin{proof}[Proof of \cref{thm:JoyalsQuillenA}, \cref{enum:Cofinal}  $\Rightarrow$ \cref{enum:WeaklyContractible} $\Rightarrow$ \cref{enum:RightAnodyne}]
	...
	Now assume \cref{enum:WeaklyContractible}. We can factor $\alpha\colon \Ii\rightarrow \Jj$ into $\Ii\rightarrow \Ii\times_{\Jj,t}\Ar(\Jj)\rightarrow \Jj$, where the first functor sends $i\in \Ii$ to the pair $(i,\id_{\alpha(i)}\colon \alpha(i)\rightarrow \alpha(i))$ and the second functor is induced by the source projection $s\colon \Ar(\Jj)\rightarrow \Jj$. It's straightforward to verify that $\Ii\rightarrow \Ii\times_{\Jj,t}\Ar(\Jj)$ is right adjoint to the projection $t\colon \Ii\times_{\Jj,t}\Ar(\Jj)\rightarrow \Ii$.\footnote{For example, one could use \cref{lem:HomInArrowCategories}; alternatively, unit and counit as well as the triangle identities are easily constructed by hand.} By \cref{exm:Cofinal}\cref{enum:RightAdjointCofinal}, we see that the functor $\Ii\rightarrow \Ii\times_{\Jj,t}\Ar(\Jj)$ satisfies \cref{enum:Cofinal}, hence also \cref{enum:RightAnodyne}. Furthermore, a slight generalisation of the dual of \cref{exm:Straightening}\cref{enum:ArCocartesianFibration} (which can be proved by the same argument) shows that $s\colon\Ii\times_{\Jj,t}\Ar(\Jj)\rightarrow \Jj$ is a cartesian fibration. Its fibres $s^{-1}\{j\}\simeq \Ii\times_\Jj \Jj_{j/}$ are weakly contractible. Hence $s\colon\Ii\times_{\Jj,t}\Ar(\Jj)\rightarrow \Jj$ satisfies \cref{enum:RightAnodyne} by \cref{lem:CartesianCofinal}. We conclude that $\alpha\colon \Ii\rightarrow \Jj$ must satisfy \cref{enum:RightAnodyne} as well. Indeed, if $f\colon X\rightarrow \Jj$ is a right fibration, then
	\begin{align*}
		\Hom_{\Cat_{\infty/\Jj}}(\Jj,X)&\simeq \Hom_{\Cat_{\infty/\Jj}}\bigl(\Ii\times_{\Jj,t}\Ar(\Jj),X\bigr)\\
		&\simeq  \Hom_{\Cat_{\infty/\Ii\times_{\Jj,t}\Ar(\Jj)}}\bigl(\Ii\times_{\Jj,t}\Ar(\Jj),s^*(X)\bigr)\,.
	\end{align*}
	In the first equivalence we use \cref{enum:RightAnodyne} for $s\colon\Ii\times_{\Jj,t}\Ar(\Jj)\rightarrow \Jj$. In the second equivalence we let $s^*(X)\rightarrow \Ii\times_{\Jj,t}\Ar(\Jj)$ be the pullback of $f$ along $s$ and use \cref{lem:KanExtensionForRight}\cref{enum:ForgetfulFunctor} below. Now a pullback of a right fibration is again a right fibration, whence
	\begin{align*}
		\Hom_{\Cat_{\infty/\Ii\times_{\Jj,t}\Ar(\Jj)}}\bigl(\Ii\times_{\Jj,t}\Ar(\Jj),s^*(X)\bigr)&\simeq \Hom_{\Cat_{\infty/\Ii\times_{\Jj,t}\Ar(\Jj)}}\bigl(\Ii,s^*(X)\bigr)\\
		&\simeq \Hom_{\Cat_{\infty/\Jj}}(\Ii,X)\,.
	\end{align*}
	In the first equivalence we use \cref{enum:RightAnodyne} for $\Ii\rightarrow \Ii\times_{\Jj,t}\Ar(\Jj)$ and in the second we use \cref{lem:KanExtensionForRight}\cref{enum:ForgetfulFunctor} below again. This finishes the proof of the implication \cref{enum:WeaklyContractible} $\Rightarrow$ \cref{enum:RightAnodyne}.
\end{proof}
...
Our first goal is to construct left Kan extensions for presheaf $\infty$-categories. As it turns out, this is most easily done in the fibration picture.
